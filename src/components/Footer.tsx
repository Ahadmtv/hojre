        ...
        <footer>
                        ...
                        <div className='flex flex-col gap-3'>
                            ...
                            <div className='flex gap-1'>
                                {socialList.map((item, i) => {
                                    return (
                                        <Link to={item.siteLink} key={i} className={`${item.className} w-[35px] h-[35px] flex justify-center items-center rounded hover:-translate-y-1 duration-200 ease-linear`}><i className={`${item.iconName}`}></i></Link>
                                    )
                                })}
                            </div>
                        </div>
                        <div className='md:pr-[25%]'>
                            ...
                            <div className=''>
                                <ul>
                                    {ItemList.map((item, i) => {
                                        return (
                                            <li className='my-1' key={i}><Link className='font-vazir-thin' to={item.link}>{item.text}</Link></li>
                                        )
                                    })}
                                </ul>
                            </div>
            ...
            <div className='footer-bot py-5 bg-gray-300'>
                <div className='container mx-auto px-10 flex justify-center items-center'>
                    <div><p className='text-center'>تمام حقوق مادی و معنوی سایت برای <Link to="/shop">"حجره"</Link> محفوظ است.| copyright &copy; 2024</p></div>
                </div>
            </div>
        </footer>
    )
